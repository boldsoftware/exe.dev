    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ fontSize: '18px', fontWeight: '600', color: '#1f2937', margin: 0 }}>Usage Details</h2>
          <button
            onClick={onClose}
            style={{
              background: 'none',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '12px 20px', fontSize: '14px' }}>
          {usage.model && (
            <>
          ...
          {usage.cache_read_input_tokens > 0 && (
            <>
              ...
              <div style={{ color: '#1f2937' }}>{usage.cache_read_input_tokens.toLocaleString()}</div>
            </>
          )}
          {usage.cache_creation_input_tokens > 0 && (
            <>
              <div style={{ color: '#6b7280', fontWeight: '500' }}>Cache Write:</div>
              <div style={{ color: '#1f2937' }}>{usage.cache_creation_input_tokens.toLocaleString()}</div>
            </>
          )}
