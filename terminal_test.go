...
func TestRealSSHIntegration(t *testing.T) {
	...
	server, err := NewServer(":18099", "", ":12346", tmpDB.Name(), "local", "")
	if err != nil {
		t.Fatalf("Failed to create server: %v", err)
	}
...
# Start SSH in background with output capture
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR -p 12346 localhost > ssh_output.txt 2>&1 <<EOF &
test@example.com
...
echo "=== SSH output captured ==="
if [ -f ssh_output.txt ]; then
    echo "Content of ssh_output.txt:"
    cat ssh_output.txt
else
...
`
	...
	if err == nil {
		...
		t.Logf("Could not read SSH output file: %v", err)
	}
}
