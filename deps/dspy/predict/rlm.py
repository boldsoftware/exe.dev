...
from ..primitives.code_interpreter import SIMPLE_TYPES, CodeInterpreter, CodeInterpreterError, FinalOutput
...
PROXY_INSTRUCTIONS = """
PROXY OBJECTS: Some inputs are proxy objects representing external data sources.
- Use `dir(obj)` to discover available attributes
- Use `obj.__doc__` to read the object's documentation
- Use `obj.__attr_docs__` to see per-attribute types and usage (e.g. whether a field is a string, list, dict, etc.) — check this BEFORE guessing attribute shapes
- Access attributes normally: `obj.name`, `obj.channels`, etc.
- Attribute access is lazy — data is fetched from the host on first access and cached
- Lists of proxy objects can be iterated: `for item in obj.items: ...`
- Concrete data (strings, numbers, etc.) is returned directly
...
def _is_proxy_object(value: Any) -> bool:
    """Check if a value is a ProxyObject (duck-typed)."""
    return hasattr(value, '_proxy_id') and hasattr(value, '_proxy_type_name')
...
class RLM(Module):
    ...
    def _build_variables(self, **input_args: Any) -> list[REPLVariable]:
        ...
        ProxyObject inputs get special treatment: instead of serializing their
        value, we show their type name, docstring, and available attributes.
        """
        variables = []
        for name, value in input_args.items():
            field_info = self.signature.input_fields.get(name)

            if _is_proxy_object(value):
                # Build a proxy-appropriate REPLVariable
                attr_list = ", ".join(value._proxy_dir) if value._proxy_dir else "(use dir() to explore)"
                doc_line = value._proxy_doc or "No documentation"
                preview = f"<{value._proxy_type_name}> — {doc_line}\nAttributes: {attr_list}"
                variables.append(REPLVariable(
                    name=name,
    ...
    def _has_proxy_inputs(self, input_args: dict[str, Any]) -> bool:
        """Check if any input arguments are proxy objects."""
    ...
    def _execute_iteration(
        ...
        # Append proxy instructions if proxy inputs are present
        extra_info = ""
        if self._has_proxy_inputs(input_args):
            extra_info = "\n" + PROXY_INSTRUCTIONS

        action = self.generate_action(
            variables_info="\n\n".join(variables_info) + extra_info,
            repl_history=history,
