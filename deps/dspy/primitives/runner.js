...
// UDS proxy bridge (Phase 2): direct socket path for proxy resolution.
// When a UDS path is provided as args[1], proxy attribute access and method
// calls bypass the stdin/stdout JSON-RPC channel and go directly over the
// Unix socket. Python-side code constructs the JSON body and specifies the
// HTTP path; this function just handles the transport.
const udsPath = Deno.args[1] || "";
if (udsPath) {
  async function udsPost(path, bodyStr) {
    ...
    const requestBytes = new Uint8Array(headerBytes.length + bodyBytes.length);
    requestBytes.set(headerBytes);
    requestBytes.set(bodyBytes, headerBytes.length);

    const conn = await Deno.connect({ transport: "unix", path: udsPath });
    try {
      ...
      // Parse status line and check for errors
      const headerEnd = responseText.indexOf("\r\n\r\n");
      if (headerEnd === -1) {
        throw new Error("Invalid HTTP response from UDS proxy server");
      }
      const statusLine = responseText.substring(0, responseText.indexOf("\r\n"));
      const statusMatch = statusLine.match(/^HTTP\/\d+\.\d+\s+(\d+)/);
      const statusCode = statusMatch ? parseInt(statusMatch[1], 10) : 0;
      const responseBody = JSON.parse(responseText.substring(headerEnd + 4));
      if (statusCode !== 200) {
        throw new Error(responseBody.error || `UDS proxy error (HTTP ${statusCode})`);
      }
      return responseBody;
    } finally {
      conn.close();
  ...
  pyodide.globals.set("_uds_post", udsPost);
}
...
while (true) {
  ...
  if (method === "execute") {
    ...
    try {
      ...
      // Get error args for exception types.
      // For SyntaxError, last_exception_args is only available if setup completed
      // (i.e. PYTHON_SETUP_CODE ran before the error). For parse-time SyntaxErrors
      // during loadPackagesFromImports, setup hasn't run yet.
      let errorArgs = [];
      if (errorType !== "SyntaxError" || setupCompleted) {
        try {
          const last_exception_args = pyodide.globals.get("last_exception_args");
          errorArgs = JSON.parse(last_exception_args()) || [];
        } catch (_) {
          // last_exception_args not available
        }
      }

      // For SyntaxError with a blank message, extract details from String(error)
      // which contains the Python traceback with line/column info.
      let finalMessage = errorMessage;
      if (!finalMessage && error) {
        const full = String(error);
        // Extract the last SyntaxError line from the traceback
        const lines = full.split("\n").filter(l => l.trim());
        const syntaxLine = lines.find(l => l.startsWith("SyntaxError:"));
        if (syntaxLine) {
          finalMessage = syntaxLine.replace("SyntaxError: ", "").trim();
        } else if (lines.length > 0) {
          // Fall back to last non-empty line
          finalMessage = lines[lines.length - 1].trim();
        }
      }

      // Map error type to JSON-RPC error code
      const errorCode = JSONRPC_APP_ERRORS[errorType] || JSONRPC_APP_ERRORS.Unknown;
      console.log(jsonrpcError(errorCode, finalMessage, requestId, { type: errorType, args: errorArgs }));
    } finally {
