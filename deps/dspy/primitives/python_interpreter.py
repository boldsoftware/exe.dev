...
- UDS proxy support (uds_path parameter, PROXY_REF_CODE injection)
- _serialize_value extended to handle ProxyObject instances
- Protocol hardening: _kill_sandbox on any protocol violation
"""
...
from .code_interpreter import SIMPLE_TYPES, CodeInterpreterError, FinalOutput
...
# =============================================================================
# Proxy Support - ProxyRef class injected into the sandbox
# =============================================================================
...
class ProxyRef:
    """Lazy proxy reference that resolves attributes via UDS to the host.

    Attribute access triggers resolution back to the host where the real
    Python object lives. Results are cached after first access. Methods
    appear as callable ProxyMethod wrappers.
    """

    def __init__(self, proxy_id, type_name, doc="", dir_attrs=None, attr_docs=None):
        object.__setattr__(self, '_proxy_id', proxy_id)
        object.__setattr__(self, '_type_name', type_name)
        object.__setattr__(self, '_doc', doc)
        object.__setattr__(self, '_dir_attrs', dir_attrs or [])
        object.__setattr__(self, '_attr_docs', attr_docs or {})
    ...
    def __getattr__(self, name):
        ...
        value = _proxy_resolve(proxy_id, name)
        cache[name] = value
        return value
    ...
    def __repr__(self):
        ...
        proxy_id = object.__getattribute__(self, '_proxy_id')
        return f"<{type_name} {proxy_id}>"

    def __dir__(self):
        return list(object.__getattribute__(self, '_dir_attrs'))
    ...
    def __iter__(self):
        proxy_id = object.__getattribute__(self, '_proxy_id')
        items = _proxy_resolve(proxy_id, "__iter__")
        return iter(items)


class ProxyMethod:
    """Callable wrapper for a proxy object method.

    Created when accessing a method name on a ProxyRef. The method itself
    runs host-side; arguments are sent over UDS and must be JSON primitives.
    """

    def __init__(self, proxy_id, method_name):
        self._proxy_id = proxy_id
        self._method_name = method_name

    def __call__(self, *args, **kwargs):
        return _proxy_call(self._proxy_id, self._method_name, list(args), kwargs)

    def __repr__(self):
        return f"<method {self._method_name} on {self._proxy_id}>"


def _unwrap_proxy_result(result):
    """Convert a classified result from the host into ProxyRef or concrete value."""
    if isinstance(result, dict):
        ...
        if t == "proxy":
            return ProxyRef(
                ...
                result.get("doc", ""),
                result.get("dir", []),
                result.get("attr_docs", {}),
            )
        elif t == "concrete":
            return result["value"]
        elif t == "proxy_list":
            return [_unwrap_proxy_result(item) for item in result["items"]]
        elif t == "proxy_dict":
            return {k: _unwrap_proxy_result(v) for k, v in result["items"].items()}
        elif t == "method":
            return ProxyMethod(result["proxy_id"], result["name"])
    return result


def _proxy_resolve(proxy_id, attr):
    """Resolve a proxy attribute via UDS."""
    body = _json.dumps({"proxy_id": proxy_id, "attr": attr})
    try:
        raw = run_sync(_uds_post("/proxy/getattr", body))
    except Exception as e:
        raise AttributeError(str(e)) from None
    if isinstance(raw, JsProxy):
        raw = raw.to_py()
    return _unwrap_proxy_result(raw)


def _proxy_call(proxy_id, method, args, kwargs):
    """Call a proxy method via UDS."""
    body = _json.dumps({"proxy_id": proxy_id, "method": method, "args": args, "kwargs": kwargs})
    try:
        raw = run_sync(_uds_post("/proxy/call", body))
    except Exception as e:
...
class PythonInterpreter:
    ...
    def __init__(
        ...
        Args:
            ...
            uds_path: Path to a Unix Domain Socket for proxy resolution.
                   When set, proxy attribute access goes over UDS to the MuxServer.
        """
        ...
        if deno_command:
            ...
            # Deno 2.x stores npm packages in node_modules/.deno/ (not in denoDir)
            pyodide_path = self._get_pyodide_npm_path()
            if pyodide_path:
                allowed_read_paths.append(pyodide_path)
            ...
            # UDS socket needs read+write permissions
            if self._uds_path:
                allowed_read_paths.append(self._uds_path)
            ...
            # Build write paths (user paths + UDS socket if present)
            write_paths = [str(x) for x in self.enable_write_paths] if self.enable_write_paths else []
            if self._uds_path:
                write_paths.append(self._uds_path)
            if write_paths:
                args.append(f"--allow-write={','.join(write_paths)}")

            args.append(self._get_runner_path())

            # Runner args: args[0] = env vars, args[1] = UDS path (optional)
            if self._uds_path:
                args.append(self._env_arg)  # always include so UDS path is at args[1]
                args.append(self._uds_path)
            elif self._env_arg:
    ...
    @staticmethod
    @functools.lru_cache(maxsize=1)
    def _get_pyodide_npm_path() -> str | None:
        """Find the pyodide npm package directory (Deno 2.x stores npm packages in node_modules/.deno/)."""
        home = pathlib.Path.home()
        node_modules = home / "node_modules" / ".deno"
        if node_modules.is_dir():
            for p in node_modules.glob("pyodide@*/node_modules/pyodide"):
                if p.is_dir():
                    return str(p)
        return None
    ...
    def _kill_sandbox(self, reason: str) -> None:
        """Kill the Deno subprocess immediately on protocol violation.

        Crash-and-restart is safer than silent tolerance — a crash is
        noticeable, a leak is not.
        """
        logger.error(f"SECURITY: killing sandbox: {reason}")
    ...
    def _serialize_value(self, value: Any) -> str:
        ...
        ProxyObjects are serialized as ProxyRef(...) constructor calls.
        """
        if value is None:
            return "None"
            ...
            return f"[{items}]"
        elif hasattr(value, '_proxy_id') and hasattr(value, '_proxy_type_name'):
            # ProxyObject — serialize as ProxyRef constructor call
            proxy_id = repr(value._proxy_id)
            type_name = repr(value._proxy_type_name)
            doc = repr(getattr(value, '_proxy_doc', ''))
            dir_attrs = repr(getattr(value, '_proxy_dir', []))
            attr_docs = repr(getattr(value, '_proxy_attr_docs', {}))
            return f"ProxyRef({proxy_id}, {type_name}, {doc}, {dir_attrs}, {attr_docs})"
        else:
            raise CodeInterpreterError(f"Unsupported value type: {type(value).__name__}")

    def _inject_proxy_support(self) -> None:
        """Inject ProxyRef class definition into the sandbox."""
        self._send_request("execute", {"code": PROXY_REF_CODE}, "injecting proxy support")
    ...
    def execute(
        ...
        # Inject proxy support if any variables are proxy objects
        has_proxies = any(
            hasattr(v, '_proxy_id') and hasattr(v, '_proxy_type_name')
        ...
        while True:
            ...
            if "method" in msg:
                if msg["method"] == "tool_call":
                    ...
                    continue
                # Any other method is a protocol violation (possible stdout injection)
                self._kill_sandbox(f"unexpected method from sandbox: {msg['method']}")
                raise CodeInterpreterError(f"Protocol violation: unexpected method '{msg['method']}'")

            # Handle success response
            if "result" in msg:
                if msg.get("id") != execute_request_id:
                    self._kill_sandbox(f"result ID mismatch: expected {execute_request_id}, got {msg.get('id')}")
                    raise CodeInterpreterError(f"Protocol violation: result ID mismatch")
                result = msg["result"]
                self._sync_files()
            ...
            if "error" in msg:
                if msg.get("id") is not None and msg.get("id") != execute_request_id:
                    self._kill_sandbox(f"error ID mismatch: expected {execute_request_id}, got {msg.get('id')}")
                    raise CodeInterpreterError(f"Protocol violation: error ID mismatch")
                error = msg["error"]
                ...
                if error_code == JSONRPC_APP_ERRORS["SyntaxError"]:
                    detail = error_message or error_data.get("args") or ""
                    raise SyntaxError(f"SyntaxError: {detail}" if detail else "SyntaxError in generated code")
                else:
                    raise CodeInterpreterError(f"{error_type}: {error_data.get('args') or error_message}")

            # Unrecognized message format — kill the sandbox
            self._kill_sandbox(f"unrecognized message format: {str(msg)[:200]}")
            raise CodeInterpreterError(f"Protocol violation: unrecognized message format")
