...
func TestIntegrationsProxy(t *testing.T) {
	...
	t.Run("team_int_no_team", func(t *testing.T) {
		// Team integration requests for a user not in a team should return 403.
		curlRetry(t, "-o /dev/null -w '%{http_code}' http://echoproxy.team.exe.cloud/", "403")
	})
