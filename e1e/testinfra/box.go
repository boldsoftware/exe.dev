...
// CreateTeamWithOwner creates a team and adds a user as owner.
// ownerEmail is used to look up the user_id.
func (se *ServerEnv) CreateTeamWithOwner(teamID, displayName, ownerEmail string) error {
	// Get owner user_id from email
	ownerUserID, err := se.GetUserIDByEmail(ownerEmail)
	if err != nil {
		return fmt.Errorf("failed to get owner user_id: %v", err)
	}

	createURL := fmt.Sprintf("http://localhost:%d/debug/teams/create", se.Exed.HTTPPort)
	resp, err := http.PostForm(createURL, url.Values{
		"team_id":       {teamID},
		"display_name":  {displayName},
		"owner_user_id": {ownerUserID},
	})
	if err != nil {
		return fmt.Errorf("failed to POST to /debug/teams/create: %v", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("POST /debug/teams/create failed with status %d: %s", resp.StatusCode, body)
	}
	return nil
}

// AddTeamMember adds a user to an existing team.
// memberEmail is used to look up the user_id.
func (se *ServerEnv) AddTeamMember(teamID, memberEmail, role string) error {
	// Get member user_id from email
	memberUserID, err := se.GetUserIDByEmail(memberEmail)
	if err != nil {
		return fmt.Errorf("failed to get member user_id: %v", err)
	}
...
// TeamMember represents a team member from the debug endpoint.
...
func (se *ServerEnv) GetTeamMembers(teamID string) ([]TeamMember, error) {
	membersURL := fmt.Sprintf("http://localhost:%d/debug/teams/members?team_id=%s", se.Exed.HTTPPort, teamID)
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return nil, fmt.Errorf("GET /debug/teams/members failed with status %d: %s", resp.StatusCode, body)
	}

	var members []TeamMember
	if err := json.NewDecoder(resp.Body).Decode(&members); err != nil {
		return nil, fmt.Errorf("failed to parse team members: %v", err)
	}
	return members, nil
}

// GetUserIDByEmail returns the user_id for a given email address.
func (se *ServerEnv) GetUserIDByEmail(email string) (string, error) {
	usersURL := fmt.Sprintf("http://localhost:%d/debug/users?format=json", se.Exed.HTTPPort)
	resp, err := http.Get(usersURL)
	if err != nil {
		return "", fmt.Errorf("failed to get users: %v", err)
