...
EXEPROX_ENABLED="${EXEPROX_ENABLED:-false}"
NUM_EXEPROXES="${NUM_EXEPROXES:-1}"
EXED_VCPUS="${EXED_VCPUS:-1}"
EXED_RAM="${EXED_RAM:-1024}"
...
scrape_configs:
  ...
  - job_name: \"exeprox\"
    ...
    local metrics_proxy_targets=("$exed_ip")
    if [[ -n "$exeprox_1_ip" ]]; then
        metrics_proxy_targets+=("$exeprox_1_ip")
    fi
    for target_ip in "${metrics_proxy_targets[@]}"; do
...
write_envfile() {
    ...
    log "Writing envfile: ${envfile}"
    {
        ...
        echo "EXEPROX_ENABLED=${EXEPROX_ENABLED}"
        if [[ "$EXEPROX_ENABLED" == "true" ]]; then
...
*)
    ...
    echo "  NUM_EXELETS=${NUM_EXELETS}  EXEPROX_ENABLED=${EXEPROX_ENABLED}  NUM_EXEPROXES=${NUM_EXEPROXES}  CLUSTER_PREFIX=${CLUSTER_PREFIX}"
