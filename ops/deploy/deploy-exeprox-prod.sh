...
if [ $# -lt 1 ] || [ -z "$1" ] || [[ "$1" == -* ]]; then
    ...
    echo "Example: $0 exeprox-lax2-prod-01" >&2
    exit 1
fi
...
# Slack notification
DEPLOY_TS=$("$REPO_ROOT/scripts/deploy-notify.sh" start exeprox "" "$INSTANCE_NAME")
cleanup_notify() {
...
echo "==========================================="
echo "Deploying exeprox to Prod"
echo "==========================================="
echo ""
...
if ! scp "ops/deploy/exeprox-prod.service" "$TAILSCALE_HOST:~/exeprox.service"; then
