#!/bin/bash
# Deploy script for exepipe binary
...
"$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/scripts/check-deploy-env.sh" staging "$INSTANCE_NAME"
...
"$REPO_ROOT/scripts/check-prodlock.sh" staging

# Slack notification
DEPLOY_TS=$("$REPO_ROOT/scripts/deploy-notify.sh" start exepipe-staging "" "$INSTANCE_NAME")
cleanup_notify() {
...
echo "==========================================="
echo "Deploying exepipe to Staging"
echo "==========================================="
echo ""
...
BINARY_NAME="exepipe.$TIMESTAMP"

echo -e "${YELLOW}Building binary...${NC}"
echo "Binary name: $BINARY_NAME"

# Build the binary
GOOS=linux GOARCH=${ARCH} CGO_ENABLED=0 go build -ldflags="-s -w" -o "/tmp/$BINARY_NAME" ./cmd/exepipe
...
if ! scp "ops/deploy/exepipe-staging.service" "$TAILSCALE_HOST:~/exepipe.service"; then
...
rm -f ~/exepipe.latest
ln -sf ~/$BINARY_NAME ~/exepipe.latest

# Install systemd service file
sudo mv ~/exepipe.service /etc/systemd/system/exepipe.service
sudo systemctl daemon-reload

# List all deployed versions
echo ""
echo "Deployed versions:"
ls -la ~/exepipe.* | tail -5
EOF
...
echo "  ls -la ~/exepipe.*  # list all versions"
echo "  sudo ln -sf ~/exepipe.TIMESTAMP ~/exepipe.latest"
echo "  sudo systemctl restart exepipe"
echo ""
echo -e "${YELLOW}==========================================="
echo -e "${NC}"
echo ""
...
ssh -o StrictHostKeyChecking=no "$TAILSCALE_HOST" <<EOF
echo "Restarting exepipe service..."
sudo systemctl restart exepipe

# Wait for service to start
sleep 3

# Check service status
if sudo systemctl is-active --quiet exepipe; then
    echo "✓ Service started successfully"
else
    echo "⚠ Service may have issues, checking logs..."
    sudo journalctl -u exepipe -n 20 --no-pager
fi

# Show recent logs
echo ""
echo "Recent service logs:"
sudo journalctl -u exepipe -n 5 --no-pager -o cat
EOF
...
echo ""
echo "View logs:"
echo "  ssh ubuntu@$INSTANCE_NAME journalctl -fu exepipe"
