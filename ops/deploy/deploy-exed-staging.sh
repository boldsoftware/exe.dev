...
DEPLOY_TS=$("$REPO_ROOT/scripts/deploy-notify.sh" start exed-staging "$OLD_SHA")
cleanup_notify() {
...
echo "==========================================="
echo "Deploying exed to Staging"
echo "==========================================="
echo ""
...
if ~/$BINARY_NAME --preflight --db /tmp/preflight.db --stage staging; then
    ...
    echo "✗ Migration preflight FAILED"
...
echo "  ssh ubuntu@$INSTANCE_NAME"
echo "  ls -la ~/exed.*  # list all binary versions"
echo "  ls -la ~/exe.db.*.sql.zst  # list all DB backups"
...
echo "Waiting for https://${DOMAIN}/health to respond..."

# Health check loop running on dev machine
for i in {1..30}; do
    if curl -s -o /dev/null -w "" https://${DOMAIN}/health 2>/dev/null; then
