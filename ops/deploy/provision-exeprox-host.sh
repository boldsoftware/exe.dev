#!/bin/bash
# Provision an exeprox host that already exists and is reachable via SSH.
# Installs Tailscale, builds and deploys sshpiper, and configures the server.
#
# This script is called by setup-exeprox-host.sh after the Latitude server
# is created, but can also be run standalone to re-provision an existing host.
...
Provision an existing bare metal exeprox host via SSH.
Installs Tailscale, builds sshpiper, and deploys it.

Example:
  $0 exeprox-nyc-prod-01 203.0.113.42
...
# SSH options for direct IP access
...
echo ""
echo "=========================================="
echo "Provisioning exeprox host: ${MACHINE_NAME}"
echo "=========================================="
echo "IP: ${SERVER_IP}"
echo ""
