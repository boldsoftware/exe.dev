...
func UnaryClientInterceptor() grpc.UnaryClientInterceptor {
	return func(ctx context.Context, method string, req, reply any, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
		...
		if traceID := ctx.Value("trace_id"); traceID != nil {
			...
			slog.DebugContext(ctx, "gRPC client: NO trace_id in context", "method", method)
		}
...
func StreamClientInterceptor() grpc.StreamClientInterceptor {
	return func(ctx context.Context, desc *grpc.StreamDesc, cc *grpc.ClientConn, method string, streamer grpc.Streamer, opts ...grpc.CallOption) (grpc.ClientStream, error) {
		...
		if traceID := ctx.Value("trace_id"); traceID != nil {
			...
			slog.DebugContext(ctx, "gRPC client stream: NO trace_id in context", "method", method)
		}
	...
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		...
		if md, ok := metadata.FromIncomingContext(ctx); ok {
			if values := md.Get(traceIDMetadataKey); len(values) > 0 && values[0] != "" {
				...
				slog.DebugContext(ctx, "gRPC server: NO trace_id in metadata", "method", info.FullMethod)
			}
		}
	...
	return func(srv any, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
		...
		if md, ok := metadata.FromIncomingContext(ctx); ok {
			if values := md.Get(traceIDMetadataKey); len(values) > 0 && values[0] != "" {
				...
				slog.DebugContext(ctx, "gRPC server stream: NO trace_id in metadata", "method", info.FullMethod)
			}
		}
