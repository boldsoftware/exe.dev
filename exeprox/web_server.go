...
func (wp *WebProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case r.URL.Path == "/debug" || strings.HasPrefix(r.URL.Path, "/debug/"):
		...
		if wp.tsDomain != "" && domz.Canonicalize(domz.StripPort(host)) == wp.tsDomain {
			if !exedebug.AllowDebugAccess(wp.env, w, r) {
				return
			}
			wp.debugHandler().ServeHTTP(w, r)
			return
		}
	}
...
func (epd *exewebProxyData) SendEmail(ctx context.Context, req email.SendRequest) error {
	return epd.exeproxData().SendEmail(ctx, req)
}
