...
func TestMultiDockerHostSSH(t *testing.T) {
	...
	// Verify the fix: machine should retain docker host info (globally unique name now)
...
func TestDockerHostPersistence(t *testing.T) {
	...
	// 3. Machine retrieved from DB with docker host info intact (globally unique name now)
	machine, err := server.getMachineByName(machineName)
	if err != nil {
		t.Fatalf("Failed to retrieve machine: %v", err)
	}
