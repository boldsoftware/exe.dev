...
func (tr *TagResolver) GetImageMetadata(ctx context.Context, registry, repository, tag, platform string) (*ImageConfig, error) {
	var imageUser, imageEntrypoint, imageCmd, imageLabels, imageExposedPorts *string
	...
	err := tr.db.Rx(ctx, func(ctx context.Context, rx *sqlite.Rx) error {
		...
		imageExposedPorts = result.ImageExposedPorts
		return nil
	})
	if err != nil {
		return nil, err
	...
	// Parse exposed ports (optional field)
	if imageExposedPorts != nil && *imageExposedPorts != "" {
		if err := json.Unmarshal([]byte(*imageExposedPorts), &cfg.ExposedPorts); err != nil {
			log.Printf("Failed to unmarshal exposed ports: %v", err)
		}
	}
...
func (tr *TagResolver) StoreImageMetadata(ctx context.Context, registry, repository, tag, platform string, cfg *ImageConfig) error {
	...
	exposedPortsJSON, err := json.Marshal(cfg.ExposedPorts)
	if err != nil {
	...
	return tr.db.Tx(ctx, func(ctx context.Context, tx *sqlite.Tx) error {
		...
		exposedPortsStr := string(exposedPortsJSON)
		if exists {
			...
			return queries.UpdateTagResolutionMetadata(ctx, exedb.UpdateTagResolutionMetadataParams{
				...
				ImageExposedPorts: &exposedPortsStr,
				UpdatedAt:         now,
				Registry:          registry,
				Repository:        repository,
				Tag:               tag,
