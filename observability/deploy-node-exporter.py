...
def load_hosts():
    ...
    Reads both static_configs and file_sd_configs (resolving the JSON files
    relative to the local repo, not /etc/prometheus/targets).
    """
    ...
    for job in config["scrape_configs"]:
        if job["job_name"] not in ("node", "exe-ops"):
            continue
        for group in job.get("static_configs") or []:
            for target in group["targets"]:
                host, port_str = target.rsplit(":", 1)
                hosts[host] = int(port_str)
        for fsd in job.get("file_sd_configs") or []:
            for pattern in fsd.get("files") or []:
                # Resolve /etc/prometheus/targets/*.json to local copies
                # generated by discover-exelets.py
                basename = os.path.basename(pattern)
                local = os.path.join(SCRIPT_DIR, "targets", basename)
                if not os.path.exists(local):
                    print(f"WARNING: {local} not found, run: scripts/discover-exelets.py targets/", file=sys.stderr)
                    continue
