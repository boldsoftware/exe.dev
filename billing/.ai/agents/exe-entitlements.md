...
- `execore/billing_status.go` -- `UserHasEntitlement`, `userNeedsBilling`, `teamBillingCovers`, plan resolution at the server layer

## Responsibilities

- Define and maintain the set of entitlements (e.g. `llm:use`, `credit:purchase`, `vm:create`)
- Define and maintain the plan catalog (VIP, Team, Individual, Friend, Grandfathered, Invite, Basic)
...
- Coordinate with **exe-credits** when entitlements gate credit operations (e.g. `credit:purchase`, `credit:renew`)
