...
// handleBillingPortal redirects users to the Stripe billing portal where they can
// manage their subscription, update payment methods, and cancel.
func (s *Server) handleBillingPortal(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.slog().ErrorContext(ctx, "failed to get user account", "error", err)
		http.Error(w, "no billing account found", http.StatusNotFound)
		return
	}

	// Require active billing status
	...
	returnURL := s.webBaseURLNoRequest() + "/user"
	portalURL, err := s.billing.PortalSession(ctx, account.ID, returnURL)
	if err != nil {
		s.slog().ErrorContext(ctx, "failed to create billing portal session", "error", err)
