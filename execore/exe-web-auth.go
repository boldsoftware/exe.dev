...
// handleBillingCheckoutSuccess handles the callback from Stripe after successful checkout.
// It creates the user account and signals the SSH session to continue.
func (s *Server) handleBillingCheckoutSuccess(w http.ResponseWriter, r *http.Request) {
	token := r.URL.Query().Get("token")
	if token == "" {
		http.Error(w, "Missing token parameter", http.StatusBadRequest)
		return
	...
	verification := s.lookUpEmailVerification(token)
	if verification == nil {
		s.slog().InfoContext(r.Context(), "billing checkout success with invalid token", "token", token)
		http.Error(w, "Invalid or expired token", http.StatusBadRequest)
		return
	}

	// Create the user with the pre-generated UserID
	user, err := s.createUserWithSSHKeyAndID(r.Context(), verification.UserID, verification.Email, verification.PublicKey)
	if err != nil {
		s.slog().ErrorContext(r.Context(), "failed to create user after billing checkout", "error", err, "token", token)
		http.Error(w, "failed to create user account", http.StatusInternalServerError)
		return
	}

	s.slackFeed.EmailVerified(r.Context(), user.UserID)
	s.slackFeed.NewUser(r.Context(), user.UserID, user.Email, "ssh")

	// Create HTTP auth cookie for this user
	cookieValue, err := s.createAuthCookie(context.WithoutCancel(r.Context()), user.UserID, r.Host)
	if err != nil {
		s.slog().ErrorContext(r.Context(), "Failed to create auth cookie after billing checkout", "error", err)
		// Continue anyway - SSH auth will still work
	} else {
		setExeAuthCookie(w, r, cookieValue)
	}
...
// handleBillingCheckoutCanceled handles the callback from Stripe when checkout is canceled.
// It signals the SSH session with an error so it can inform the user.
func (s *Server) handleBillingCheckoutCanceled(w http.ResponseWriter, r *http.Request) {
	token := r.URL.Query().Get("token")
	if token == "" {
		http.Error(w, "Missing token parameter", http.StatusBadRequest)
		return
	...
	verification := s.lookUpEmailVerification(token)
	if verification == nil {
		s.slog().InfoContext(r.Context(), "billing checkout canceled with invalid token", "token", token)
		http.Error(w, "Invalid or expired token", http.StatusBadRequest)
		return
	}

	// Set error and signal the SSH session
	verification.Error = errors.New("subscription checkout was canceled")
	close(verification.CompleteChan)

	// Clean up email verification
	s.deleteEmailVerification(verification)

	// Show a simple canceled page
...
<p>Your subscription checkout was canceled. You can close this window and try again from your SSH session.</p>
</body>
</html>`)
}

// handleBillingCheckoutStart generates a Stripe checkout URL and redirects to it.
// This is called when the user clicks the "Complete Registration" button.
func (s *Server) handleBillingCheckoutStart(w http.ResponseWriter, r *http.Request) {
	token := r.URL.Query().Get("token")
	if token == "" {
		http.Error(w, "Missing token parameter", http.StatusBadRequest)
		return
	...
	verification := s.lookUpEmailVerification(token)
	if verification == nil {
		s.slog().InfoContext(r.Context(), "billing checkout start with invalid token", "token", token)
		http.Error(w, "Invalid or expired token", http.StatusBadRequest)
		return
	}

	if verification.UserID == "" {
		s.slog().ErrorContext(r.Context(), "billing checkout start with no user ID", "token", token)
		http.Error(w, "Invalid verification state", http.StatusInternalServerError)
		return
	}

	// Create the account before calling Stripe. The account ID is used as the
	// Stripe customer ID.
	...
	successURL := baseURL + "/billing/checkout/success?token=" + url.QueryEscape(token)
	cancelURL := baseURL + "/billing/checkout/canceled?token=" + url.QueryEscape(token)

	checkoutURL, err := s.billing.Subscribe(r.Context(), verification.AccountID, &billing.SubscribeParams{
