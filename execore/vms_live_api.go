...
type vmsLiveVM struct {
	...
	// minus host page cache attributed to the VM (memory.stat "file"). Page
	// cache from VM disk I/O is reclaimable and not part of the guest working
	// set; charging it to the VM dramatically overstates real usage. The raw
	// breakdown remains available in mem_anon_bytes / mem_file_bytes / etc.
