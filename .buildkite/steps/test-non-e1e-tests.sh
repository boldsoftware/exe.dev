...
case "$UNIT_TEST_SHARD" in
  A) PKGS="exe.dev/execore"; RUN_FILTER="-run ^Test[A-Ha-h]" ;;
  B) PKGS=$(echo "$ALL_PKGS" | grep -v '^exe\.dev/execore$') ;;
  C) PKGS="exe.dev/execore"; RUN_FILTER="-run ^Test[I-Pi-p]" ;;
  D) PKGS="exe.dev/execore"; RUN_FILTER="-run ^Test[Q-Zq-z]" ;;
  "")PKGS="$ALL_PKGS" ;;
  *) echo "Unknown UNIT_TEST_SHARD=$UNIT_TEST_SHARD (expected A-D or unset)"; exit 1 ;;
esac
