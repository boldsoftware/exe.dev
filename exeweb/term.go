...
func (ps *ProxyServer) withTerminalAuth(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err == nil && (!exists || box.CreatedByUserID != userID) {
			err = fmt.Errorf("VM '%s' not found or access denied", boxName)
		}
		if err != nil {
			// User doesn't have access to this box (or it doesn't exist)
			// Show access denied page
			dashboardURL := ps.webBaseURLNoRequest()
			data := struct {
				BoxName      string
				DashboardURL string
			}{
...
func (ps *ProxyServer) handleTerminalPage(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
	...
	ps.serveStaticFile(w, r, "terminal.html")
}

// handleTerminalWebSocket handles WebSocket connections for terminal sessions
func (ps *ProxyServer) handleTerminalWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	if !exists {
		...
		newSession, err := ps.createTerminalSession(r.Context(), userID, boxName, sessionName, workingDir)
		if err != nil {
	...
	for {
		...
		switch msg.Type {
		case "resize":
			ps.Lg.InfoContext(ctx, "Terminal resize", "cols", msg.Cols, "rows", msg.Rows)
			if needsExtraResize {
				...
				if err != nil {
					ps.Lg.WarnContext(ctx, "Failed extra resize", "error", err)
				}
				select {
				case <-ctx.Done():
					return
			...
			if msg.Cols > 0 && msg.Rows > 0 {
				...
				if session.sshSession != nil {
					...
					if err != nil {
						ps.Lg.WarnContext(ctx, "Failed resize", "error", err)
					}
				}
			}
		case "input":
...
func (ps *ProxyServer) createTerminalSession(ctx context.Context, userID, boxName, sessionName, workingDir string) (*TerminalSession, error) {
	...
	err = ps.createContainerExecSession(session, &box, sessionName, workingDir)
	if err != nil {
...
func (ps *ProxyServer) createContainerExecSession(session *TerminalSession, box *BoxData, sessionName, workingDir string) error {
	if len(box.SSHClientPrivateKey) == 0 || box.SSHPort == 0 || box.SSHUser == "" {
	...
	go ps.readFromSSHSessionAndBroadcast(session, stdout, stderr)
	return nil
}

func (ps *ProxyServer) readFromSSHSessionAndBroadcast(session *TerminalSession, stdout, stderr io.Reader) {
	defer func() {
		cleanupTerminalSession(session)
	}()
...
func (ps *ProxyServer) HandleTerminalRequest(w http.ResponseWriter, r *http.Request) {
	...
	case path == "/":
		...
		ps.withTerminalAuth(ps.handleTerminalPage)(w, r)
	...
	case strings.HasPrefix(path, "/static/"):
		...
		ps.serveStaticFile(w, r, filename)
	default:
		http.NotFound(w, r)
	}
}
