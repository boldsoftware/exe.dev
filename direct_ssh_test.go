...
// TestSCPAccess tests SCP file transfer to machines
func TestSCPAccess(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	teamName := "testteam"
	machineName := "delta-dog"
	containerID := "test-container-scp"
	...
	// SCP execution will be handled by the mock manager
	...
	// Connect and simulate SCP protocol
	client, err := ssh.Dial("tcp", listener.Addr().String(), clientConfig)
	if err != nil {
		t.Fatalf("Failed to connect: %v", err)
	}
	...
	// Test SCP target mode command (what 'scp file dest' sends)
	scpCommand := "scp -t /tmp/testfile.txt"
	output, err := session.CombinedOutput(scpCommand)
	
	// SCP should fail cleanly when openssh-client is not available
	// but the SSH session itself should not break
	if err != nil {
		// This is expected since mock containers don't have openssh-client
		t.Logf("SCP command failed as expected (no openssh-client in mock): %v", err)
	}

	// For SCP to work properly, the output should be minimal/empty or SCP protocol responses
	// The key is that it should NOT contain shell prompts or other verbose output
	outputStr := string(output)
	if strings.Contains(outputStr, "Executed:") || strings.Contains(outputStr, "root@") || strings.Contains(outputStr, "$") || strings.Contains(outputStr, "#") {
		t.Errorf("SCP command returned shell-like output which breaks SCP protocol: %q", outputStr)
	}
	
	// The output should contain the SCP protocol OK byte (0) or be empty
	if len(output) > 0 {
		// First byte should be 0 (OK) for SCP protocol
		if output[0] != 0 {
			t.Logf("SCP protocol response: first byte = %d (should be 0 for OK)", output[0])
		}
	}

	// Verify that our SCP handler did try to execute scp in the container first
	// This is the correct behavior - try real scp, fall back to error if not available
	execCalls := mockManager.GetExecCalls()
	scpFound := false
	for _, call := range execCalls {
		if call.ContainerID == containerID {
			...
			if strings.Contains(actualCommand, "scp -t") {
				scpFound = true
				t.Logf("SCP command correctly attempted in container: %s", actualCommand)
				break
			}
		}
	}
	if !scpFound {
		t.Errorf("SCP handler should try to execute scp in container first (correct behavior)")
	}
}
